node {
	def app
	def image = 'careydevelopment/ecosystem-product-service'
	...
	try {
		stage('Clone repository') {
	    	git branch: branch,
	        	...
	        	url: 'https://github.com/careydevelopment/ecosystem-product-service.git'
	    } 
	
		stage('Build JAR') {
	    	docker.image('maven:3.6.3-jdk-11').inside('-v /root/.m2:/root/.m2') {
	        	sh 'mvn -B -Dmaven.test.skip=true clean package'
	        	stash includes: '**/target/ecosystem-product-service.jar', name: 'jar'
	    	}
	    }
